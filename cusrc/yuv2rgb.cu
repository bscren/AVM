...
__global__ inline void gpuConvertYUV4202RGB_kernel(uchar* src, uchar* dst, uint width, uint height)
{
      const int x = (blockIdx.x * blockDim.x) + threadIdx.x;
      const int y = (blockIdx.y * blockDim.y) + threadIdx.y;
...
void gpuConvertYUV4202RGB_async(uchar* src, uchar* d_src, uchar* dst, uint width, uint height, cudaStream_t stream)
{
	size_t planeSize = width * height * sizeof(uchar);
...
//   cudaMemcpyAsync(d_src, src, planeSize * 2, cudaMemcpyHostToDevice, stream);
	...
	gpuConvertYUV4202RGB_kernel <<<grid, block, 0, stream>>>(d_src, dst, width, height);
